...
(
	...
	ON DELETE CASCADE
	ON UPDATE CASCADE,
	FOREIGN KEY (Administrator) REFERENCES Administrator(Login)
	ON DELETE CASCADE
	ON UPDATE CASCADE,
...
DROP FUNCTION IF EXISTS CheckAdmCred(VARCHAR(20), VARCHAR(40));
--stored procedure to check admin credentials
CREATE OR REPLACE FUNCTION CheckAdmCred (VARCHAR(20), VARCHAR(40))
  RETURNS TABLE 
  (FirstName		VARCHAR(20),
	LastName		VARCHAR(20),
	Email			VARCHAR(50),
	Remuneration	float) AS 
$func$
BEGIN
   RETURN QUERY
   SELECT a.FirstName, a.LastName, a.Email, a.Remuneration::float FROM Administrator a WHERE login = $1 and password = $2;
END
$func$ LANGUAGE plpgsql;

DROP FUNCTION IF EXISTS UpdateInstruction (
  _instructionid INTEGER,
  _amount DECIMAL(12,2),
  _frequency VARCHAR(5),
  _expirydate DATE,
  _customer VARCHAR(20),
  _administrator VARCHAR(20), 
  _etf VARCHAR(5), 
  _notes VARCHAR(200)
);
CREATE OR REPLACE FUNCTION UpdateInstruction (
  _instructionid INTEGER,
  _amount DECIMAL(12,2),
  _frequency VARCHAR(5),
  _expirydate DATE,
...
BEGIN
  ...
  SET 
      Amount = _amount, 
      Frequency = F.FrequencyCode,
      ExpiryDate = _expirydate, 
      Customer = LOWER(_customer), 
      Administrator = _administrator, 
      Code = UPPER(_etf), 
      Notes = _notes 
  FROM Frequency F
  WHERE I.InstructionId = _instructionid AND UPPER(F.FrequencyDesc) = UPPER(_frequency);
END
$$LANGUAGE plpgsql;
